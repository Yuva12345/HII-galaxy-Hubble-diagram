...
def loglikelihood(theta, z_h2gx,zerr_h2gx, logsigma_h2gx,logsigmaerr_h2gx,logf_h2gx,logferr_h2gx,logL_anc, logLerr_anc, logsigma_anc,logsigmaerr_anc):
    alpha,beta,H0,Omega_m,sigma_int = theta
    cosmo = FlatLambdaCDM(H0*u.km/u.s/u.Mpc,Omega_m)
    def Dl(z):
...
def logprior(theta):
    ...
    if not (33 <= alpha <= 35 and 4 <= beta <= 5 and  60<= H0 <=110 and 0 <= Omega_m <= 1 and 0<=sigma_int<=1):
        return -np.inf
    return 0
...
inisamples = np.array([alpha_init,beta_init, H0_init, Omegam_init,sigma_int_init]).T
...
alpha_acl, beta_acl, H0_acl, Omegam_acl, sigma_acl = sampler.get_autocorr_time(quiet=True)

samples_emcee = sampler.get_chain(flat=True, discard=Nburnin, thin=int(max([alpha_acl, beta_acl, H0_acl, Omegam_acl,sigma_acl])))
...
names = ['alpha', 'beta', 'h0','omega_m','sigma_int']
labels = [
r'\alpha',
r'\beta',
...
blue = '#1f77b4' 
g.triangle_plot([gdsamples], ['alpha', 'beta', 'h0','omega_m','sigma_int'], filled=True, contour_colors=[blue],line_args=[{'ls': '-', 'color': blue}],markers=None)
fig = plt.gcf()
ax = plt.gca()
...
sigma_int, sigma_int_hi, sigma_int_lo = median_and_cred(samples_emcee[:, 4])
...
'''theta_median = [alpha,beta,h0,omega_m,sigma_int]
