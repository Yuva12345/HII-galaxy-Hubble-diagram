...
def logprior(theta):
    ...
    if not (33 <= alpha <= 35 and 4 <= beta <= 5 and  60<= H0 <=110 and 0 <= Omega_m <= 1):
        return -np.inf
    return 0
...
inisamples = np.array([alpha_init,beta_init, H0_init, Omegam_init]).T
...
alpha_acl, beta_acl, H0_acl, Omegam_acl = sampler.get_autocorr_time(quiet=True)

samples_emcee = sampler.get_chain(flat=True, discard=Nburnin, thin=int(max([alpha_acl, beta_acl, H0_acl, Omegam_acl])))
...
'''
theta_median = [alpha,beta,h0,omega_m]
