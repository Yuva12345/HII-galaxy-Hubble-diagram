...
def prior_transform(theta): 
    alphaprime,betaprime,H0prime,Omega_mprime = theta
    min1 = 33
    max1 = 35
    min2 = 4
    max2 = 5
...
def loglikelihood_dynesty(theta):
    ...
    l_h2gx = -0.5*195*np.log(2*np.pi)-np.log(np.prod(eps_h2gx2))-0.5*np.sum((mu_obs-mu_th)**2/(eps_h2gx2))
    l_anc= -0.5*36*np.log(2*np.pi)-np.log(np.prod(eps_anc2))-0.5*np.sum((logL_anc-beta*logsigma_anc-alpha)**2/(eps_anc2))
    total = l_h2gx + l_anc
    return total
