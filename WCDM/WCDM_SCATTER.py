...
data1 = df.to_numpy()
logsigma_anc     = data1[:, 3].astype(float)
logsigmaerr_anc  = data1[:, 4].astype(float)
logL_anc         = data1[:, 5].astype(float)
logLerr_anc      = data1[:, 6].astype(float)
arr_loglike = []
...
def logprior(theta):
    ...
    if not (33 <= alpha <= 35 and 4 <= beta <= 5 and  60<= H0 <=110 and 0 <= Omega_m <= 1 and -3<=w<=0 and 0<=sigma_int<=1):
        return -np.inf
    return 0
...
alpha_init = np.random.uniform(33, 35, Nens) # 0.5 to 2
beta_init = np.random.uniform(4, 5, Nens) # 2.0 to 4.0
H0_init = np.random.uniform(60, 110, Nens) # 50 to 100
Omegam_init = np.random.uniform(0,1,Nens) # 0.0 to 1.0
w_init = np.random.uniform(-3,0,Nens)
sigma_int_init=np.random.uniform(0,1,Nens)
inisamples = np.array([alpha_init,beta_init, H0_init, Omegam_init,w_init,sigma_int_init]).T
...
alpha_acl, beta_acl, H0_acl, Omegam_acl, w_acl,sigma_acl = sampler.get_autocorr_time(quiet=True)

samples_emcee = sampler.get_chain(flat=True, discard=Nburnin, thin=int(max([alpha_acl, beta_acl, H0_acl, Omegam_acl,w_acl,sigma_acl])))
...
names = ['alpha', 'beta', 'h0','omega_m','w','sigma_int']
labels = [
r'\alpha',
r'\beta',
r'\mathcal{H}_{\mathrm{0}}',
r'\mathcal{Omega}_{\mathrm{m}}',
r'\mathcal{w}_{\mathrm{de}}',
r'\sigma_{\mathrm{int}}'
]
...
g.triangle_plot(
    [gdsamples],
    ['alpha', 'beta', 'h0','omega_m','w','sigma_int'],
    filled=True,
    contour_colors=[blue],
...
omega_m, omega_m_hi, omega_m_lo = median_and_cred(samples_emcee[:, 3])
w, w_hi, w_lo = median_and_cred(samples_emcee[:, 4])
sigma_int, sigma_int_hi, sigma_int_lo = median_and_cred(samples_emcee[:, 5])
...
print(f"omega_m: {omega_m:.2f} +{omega_m_hi:.2f} -{omega_m_lo:.2f}")
print(f"w_de: {w:.2f} +{w_hi:.2f} -{w_lo:.2f}")
