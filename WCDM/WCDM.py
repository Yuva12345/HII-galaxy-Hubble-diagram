...
def loglikelihood(theta, z_h2gx,zerr_h2gx, logsigma_h2gx,logsigmaerr_h2gx,logf_h2gx,logferr_h2gx,logL_anc, logLerr_anc, logsigma_anc,logsigmaerr_anc):
    ...
    mu_th = 5*np.log10(Dl_z)+25 
    mu_obs=2.5*(beta*logsigma_h2gx+alpha-logf_h2gx)-100.2
    sigma_obs=2.5*np.sqrt(beta**2*logsigmaerr_h2gx**2+logferr_h2gx**2)
    
    eps_h2gx2=sigma_obs**2+(5*Dl_err/(np.log(10)*Dl_z))**2
    eps_anc2 = logLerr_anc**2 + beta**2*logsigmaerr_anc**2
...
def logprior(theta):
    ...
    if not (33 <= alpha <= 35 and 4 <= beta <= 5 and  60<= H0 <=110 and 0 <= Omega_m <= 1 and -3<=w<=0):
        return -np.inf
    return 0
...
Omegam_init = np.random.uniform(0,1,Nens) 
w_init = np.random.uniform(-3,0,Nens)
inisamples = np.array([alpha_init,beta_init, H0_init, Omegam_init,w_init]).T
...
alpha_acl, beta_acl, H0_acl, Omegam_acl, w_acl = sampler.get_autocorr_time(quiet=True)

samples_emcee = sampler.get_chain(flat=True, discard=Nburnin, thin=int(max([alpha_acl, beta_acl, H0_acl, Omegam_acl,w_acl])))
...
names = ['alpha', 'beta', 'h0', 'omega_m','wde']
labels = [
    r'\alpha',
    r'\beta',
    r'H_{0}\,[\mathrm{km\,s^{-1}\,Mpc^{-1}}]',
    r'\Omega_{\mathrm{m}}',
    r'w_{\mathrm{de}}'
]
...
# --- BIC Calculation ---
'''theta_median = [alpha,beta,h0,omega_m,w]
loglike_val=loglikelihood(theta_median, z_h2gx,zerr_h2gx, logsigma_h2gx,logsigmaerr_h2gx,logf_h2gx,logferr_h2gx,logL_anc, logLerr_anc, logsigma_anc,logsigmaerr_anc)
print(loglike_val)'''
n_params = ndims
